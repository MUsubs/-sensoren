...
void setup() {
    ...
    auto vlc_sender_test_task_return =
        xTaskCreate( vlc_sender_test_task, "VLC_SENDER_TEST", 2000, NULL, 1,
                     &vlc_sender_test_task_handle );
...
void loop() {
    if ( Serial.available() > 0 ) {
        ...
        Serial.printf( "==INPUT== Setting VLC Sender Frequency to %d\n",
                       incoming_data.toInt() );
        vlc_sender.setFrequency( incoming_data.toInt() );
    }
    yield();
}